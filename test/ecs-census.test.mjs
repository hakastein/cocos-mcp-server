 ...
 * kit missing one writer must FAIL the census, and the shapes that legitimately supply a writer
 * (an object literal at assembly, a command buffer, a key-forwarding wrapper) must all count.
 */
import test from 'node:test';
import assert from 'node:assert/strict';

import census from '../dist/ecs-census.js';

const { runCensus } = census;
