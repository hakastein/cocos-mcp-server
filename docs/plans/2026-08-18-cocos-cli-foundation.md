...
**Ловушка, которую код обязан закрыть.** Функция отправки не должна рапортовать об успехе в
мёртвый сокет: `rejectAllPendingRequests` срабатывает один раз, на событии `close`, и разбирает
только то, что висело в тот момент. Вызов, сделанный после закрытия, иначе ждёт ответа, которого
не будет, а второго `close` уже не произойдёт — команда виснет без кода выхода. Держи признак
закрытости и отклоняй отправку в закрытое соединение.
