/** Copied from `source/reference-projection.ts`, which stays with the tool registry moving to `cli/`. */
...
/** Serialized entry index -> live node; unmapped means "not established", never "points at nothing" — a prefab instance ROOT carries no `_id` to key off. */
